...
public class Launcher : MonoBehaviour
{
    ...
    [Tooltip("If enabled your installer will use its own folder as workspace.")]
    ...
    [Tooltip("Enables WebRequests or FTPRequests with credentials. Generally, you need this when your remote directories are protected by login or your remote URLs are FTP ones!")]
    ...
    void OnDownloadProgress(long currentFileSize, long totalFileSize, int percentageCompleted)
    {
        ...
        UnityThreadHelper.Dispatcher.Dispatch(() =>
        {
	        DetailBar.Progress = (float) percentageCompleted;// / 100f;
            DetailBar.UpdateProgressBar();
	...
	protected void OverrideSettings(bool overrideAppPath = true)
    {
		...
		{
#if UNITY_EDITOR
            SettingsManager.APP_PATH = Directory.GetParent(Application.dataPath).FullName + Path.DirectorySeparatorChar + "PATCH" + Path.DirectorySeparatorChar + "TEST";
        ...
        SettingsManager.INSTALL_IN_LOCAL_PATH = UseLocalPath;
	    ...
	    SettingsManager.IS_INTEGRATED = IsIntegrated;
    }
	...
	{
		var shortcutFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), SettingsManager.LAUNCHER_NAME + ".url");
		var targetFile = Path.Combine(SettingsManager.APP_PATH, SettingsManager.LAUNCHER_NAME);
		File.Delete(shortcutFile);
		using (FileStream fs = new FileStream(shortcutFile, FileMode.Create, FileAccess.ReadWrite))
		{
			using (StreamWriter writer = new StreamWriter(fs))
			{
				string app = targetFile;
	...
	protected void EnsureExecutePrivileges()
	{
		...
		System.Diagnostics.Process processChmod = new System.Diagnostics.Process();
		processChmod.StartInfo.FileName = "chmod";
		processChmod.StartInfo.Arguments = "+x \"" + file + "/Contents/MacOS/" + file.Replace(".app", "") + "\"";
		processChmod.Start();
		
		System.Diagnostics.Process processAttr = new System.Diagnostics.Process();
		processAttr.StartInfo.FileName = "xattr";
		processAttr.StartInfo.Arguments = "-d com.apple.quarantine \"" + file + "\"";
		processAttr.Start();
		#endif
	}
}
